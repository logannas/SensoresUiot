...
int BaseProtocol_esp32::create_service(int number, const char *name, String unit, bool numeric, String parameter){
...
bool BaseProtocol_esp32::send_data(int service, float *data, int array_size, int sensitive=0) {
...
bool BaseProtocol_esp32::send_data(int service, char *char_data, int sensitive=0) {
	this->DEVICE_REGISTERED = (!this->DEVICE_REGISTERED)? this->register_all(service, char_data, sensitive) : this->register_service_data(service, char_data, sensitive);
  return this->DEVICE_REGISTERED;
}

void BaseProtocol_esp32::device_identificator(){
...
bool BaseProtocol_esp32::register_all(int service, char *data, int sensitive){
	return this->register_device() && this->register_service_data(service, data, sensitive);
}

bool BaseProtocol_esp32::register_service_data(int service,  char* data, int sensitive){
...
char *BaseProtocol_esp32::make_client_data(char* json){
...
char *BaseProtocol_esp32::make_service_data(Service service, char* json){
...
char *BaseProtocol_esp32::make_raw_data(Service s, char *data, int sensitive, char* json){
...
char* BaseProtocol_esp32::append_json(char *json, const char* key, const char* value, int numeric){
  // Serial.println(strlen(json));
...
char* BaseProtocol_esp32::float_to_char(float* float_array, int array_size){
  char *value;
  ...
  for(int i = 0; i < array_size; i++){
    ...
    contador += b.length() + 1;
    value = (char*) realloc (value, (contador ) * sizeof(char));
    strcat(value, b.c_str());
    strcat(value, ",");
    // Serial.print("value: ");
...
char BaseProtocol_esp32::nibble_to_char(int nibble){
  return nibble + 48 + 7*(nibble > 9); //ascii based
}


byte BaseProtocol_esp32::get_value_from_char(char hexa){
    return hexa - 48  - 7*(hexa>='A');

}
bool BaseProtocol_esp32::register_service(int service){
  return false;
}
bool BaseProtocol_esp32::register_device(){
  return false;
}
bool BaseProtocol_esp32::register_data(int svc, char* dt, int stv){
  return false;
}
